...
Last, but not least, I would like to thank all the people I have met in the last five years. You guys rock (sometime).
...
Peers in a blockchain network does not usually need to be pursue any registration as no PKI infrastructure exists. This fact is perfect to increase the level of decentralization, but it also introduces no way of punishing a misbehaving peer, as he can easily change his identity.
The most dangerous attacks on this kind of p2p networks are rational attacks, Sybil attack, and Eclipse attacks. \\
The first one includes all those scenarios where a peer acts selfishly, increasing the load of other more altruistic peers. An example is a peer who always asks for updates without sharing anything, minimizing the use of his resources and maximizing his gain. \\
A Sybil attack is a kind of security threat where one person tries to take over the network by creating multiple identities. In a blockchain-context, a malicious actor could create a high number of nodes polluting most of the network's view of other peers. This scenario could lead to a bad situation where most of the peers would not be able to receive or transmit information. \\
...
Traditionally, fault-tolerance theory admits three different models to describe actor's behaviours, determining how much the system can withstand part of it deviating from the protocol. \\
The first model is the simple fault-tolerance, where the idea is that each actor can either precisely follow the protocol or fail, meaning that the system has to be able to detect and recover from crashes and failures. This model is most useful in those scenarios where the system is architecturally decentralized but politically centralized (ex. Google cloud hosting). \\
However, what if the considered system is managed by several organizations or individuals? In this case, the model has to be exchanged for the Byzantine fault-tolerance, which states that most of the nodes will follow the protocol, but some of them will deviate either by acting maliciously or because they crash/fail. Byzantine fault-tolerance systems need to survive against a percentage of such byzantine members. \\
The last, more realistic and complex model extends the notion of Byzantine fault-tolerance; is called Byzantine Altruistic Rational model an differs from the former by adding a simple reasoning: considering a real-life scenario, every actor's behaviour in a system is determined by the incentives he gets when following the protocol. Most members of the system will always try to maximize their utility function even if this requires to deviate from the protocol. A simple example are push and pull forms of gossip: as peers are not obligated to exchange information in a Tic-For-Tac fashion, they can ask for updates and never share them, increasing the burden on a smaller subset of peers. \\
The BAR model assumes three types of actors:
\begin{itemize}
...
Looking at the literature, several gossip protocol, assuming the BAR model, have already been studied. One of the most famous is the work done in BAR Gossip\citep{li2006bar}, which developed the first data streaming application with predictable throughput and low latency. \\
 In BAR Gossip a central tracker distributes updates to a subset of peers, who will then disseminate them with other members of the network in such a way that every peer action is accountable and provable. As to our knowledge, there is no work on gossip protocols assuming a BAR model in a blockchain-context.
\section{BAR Gossip in details}
BAR Gossip is one of the first research on gossip protocols where peers follow the BAR model. It was mainly designed for streaming-context applications and provides a "fairly novel exchange" which avoids the needs of reputation systems and improves the proprieties of the network in presence of rational or byzantine actors. \\
BAR Gossip relies on a central tracker which distributes a some updates during an epoch and keep tracks of peers identity in the network. \\
...
As we want to include the same membership mechanism and exchange protocol, we are faced with some problems. Blockchain network are by nature decentralized, but BAR Gossip exploits a central tracker to distribute and keep tracks of peer. Moreover, peers in a blockchain network have different demands, with respect to streaming application, in term of data to exchange. \\
In the next chapter we look at how we managed to overcome these challenges.

\chapter{BBAR Gossip}
After having discussed the needed background, we can now move onto describing our BBAR Gossip protocol.\\
 It is important to remember that the primary goal of the protocol is to implement a more reliable p2p network able to withstand or minimize the attack surfaces introduced by p2p protocols. This goal must be persuaded, taking into account that decentralization and convergence rate are crucial factors in a blockchain network, meaning it could severely impact overall performances.\\
This chapter starts describing the model assumptions. It then moves to describe the several kinds of peers in the network and their relationship. The last three sections outlines the actual protocol with a focus on implementation details and further optimizations.

\section{Model assumptions}
The model we are using makes the following assumptions:
\begin{itemize}
	\item The p2p network is not built upon any structure.
	\item Every peer in the network adheres to the BAR model.
	\item There exist two kinds of peers: validators and full nodes.
...
Hash functions are any function which can be used to map arbitrary length data onto fixed size data. We need hashing function with three proprieties:
\begin{itemize}
	\item Pre-image resistance: given a hash value $h$ it should be difficult to find any message $m$ such that $h = hash(m)$.
	\item Second-image resistance: given an input $m1$, it should be difficult to find a different input $m2$ such that $hash(m1) = hash(m2)$
	\item Collision resistance: it should be difficult to find two different messages $m1$ and $m2$ such that $hash(m1) = hash(m2)$.
\end{itemize}
In addition to their normal use, we will exploit hash function to build cryptographic hash-based proof-of-work, as Hashcash\citep{back2002hashcash}. Hashcash PoWs, are resource intensive to compute but easy to verify and provide a valid mechanism to limit spam attacks. \\
...
Asymmetric encryption uses two key, one called private and one public, to either encrypt/decrypt or perform digital signatures. Digital signatures are a mathematical technique to validate the authenticity and integrity of a message and are commonly used to acknowledge the consent of the signer.  \\
Our implementation uses Elliptic Curve cryptography.
Pseudo-random number generators are algorithms that deterministically generates a sequence of numbers whose properties approximate the properties of random number sequences. PRNGs cannot be considered really random as their output is totally determined by an initial value called seed. Our implementation follows the Mersenne Twister PRNG, but the XORShift PRNG family is also a good candidate.
...
\begin{itemize}
	\item Bootstrap nodes are the main difference with traditional gossips. Their primary purpose is to register and keep track of new peers, remove malicious peer, and coordinate the exchange of messages. Each Bootstrap node identity is tied to a corresponding validator's identity as they can be made accountable for their actions and, in case of misbehaviour, be slashed.
	\item Entry nodes are an additional component of every bootstrap node. They act as reverse proxies to keep the network identity (IP address) of the bootstrap node hidden to avoid DDoS related attacks.
...
We have divided the protocol into three phases, that a peer has to follow to start exchanging messages and sync his state. 
...
The first thing a peer has to do to be included in a network is to contact a Seed node to receive some Bootstrap's node entry node addresses. From the list of addresses, the peer will randomly contact a subset of them and start the following message exchange.
...
\begin{itemize}
	...
	\item As soon as the peer finds a solution for the crypto-puzzle, it sends to the entry node/s a LOGIN message[\ref{tab:login-message}] with the solution.
...
Instead of randomly selecting a peer in the peer list, the selection is computed using a PNRG seeded with the token received. The output of the PNRG modulus the size of the peer list will generate the index of the selected partner in the list. \\ If the index number is the peer itself, it generates the next random number using the same PRNG, until a valid one comes out. \\
...
\begin{itemize}
	...
	\item On receiving the HISTORYDIVULGE message, the peer computes the set difference with his mempool/state. Based on those results, it decided if his best choice is a BAR or OPT exchange and sends to his partner an EXCHANGE[\ref{tab:exchange-bar-message}] message containing the exchange type to perform and a list of updates he wants and promise.
	\item The partner checks if the exchange is valid (it will be explained later what it means), and sends back a PROMISE[\ref{tab:promise-bar-message}] message, which state the fact that he accepted the exchange, and a BRIEFCASE message, which contains the updates encrypted.
	...
	\item The KEY[\ref{tab:key-bar-message}] message includes the secret key to open the corresponding briefcase, containing the requested updates.
\end{itemize}

First of all, each message sent during this phase contains a header with the token plus the relative information to verify his validity and a hash of the previous message, so that it is possible to rebuild the message history. \\
A crucial aspect of the protocol, which is heavily inspired by BAR Gossip, are the two types of exchanges available: BAL or OPT. A BAL exchange is characterized by the fact each member of the trade promised to exchange the same number of updates, in a Tic-For-Tat fashion. As this is not always possible, OPT provided a safe-net mechanism for peers who do not have something to exchange, and they rely on the altruism of other peers. In fact, a peer A who starts an OPT exchange, promises to the other 0 or more updates and asks the partner to send back either the same number of updates as him or more. 
Mooreover, as we want to incentivize the more fair BAL exchange, partner B will add a percentage of random data to the BRIEFCASE message.  \\
Another fact is that the maximum number of sharable updates in each trade is bounded both for BAL and OPT, as this increase the possibility for a peer to start a BAL exchange. \\
...
Last, but not least, as in BAR Gossip, all messages are sent through TCP connection expect the KEYREQUEST and KEY message, which are sent via UDP.
...
An important factor for this protocol is that peers need to be registered with multiple Bootstrap nodes. This is because there has to be at least a path which links all the Bootstrap node sub-networks into a single one. Otherwise, peers would be able to exchange only the information inside a single bootstrap network, or inside a subset of the entire network. \\
...
The reason why a Bootstrap node has to give the secret key for encrypting the briefcase together with the token is that a malicious peer could create some fake updates identifiers to perform a BAL exchange, wait for the partner KEY message and then fake a crash or timeout. Using this strategy, one of the peers receives the updates; meanwhile, the other would not be able to decrypt the BRIEFCASE, which contains garbage data. This problem can be reduced to the fair exchange problem, which is actually impossible to solve without a trusted third party\citep{pagnia1999impossibility}. In our case, the Bootstrap nodes act as a TTP. If a peer does not receive the KEY message, he can send an MPoM (Maybe-Proof-of-Misbehavior) with the encrypted briefcase. The bootstrap node will decrypt the BRIEFCASE message, checks the validity of the updates, and if the updates are legit, sends back the key to the peer. Otherwise, it will ban the peer's partner.

\subsection{PoM rewards}
As sending PoM is resource expensive for peers, we need to incentivize this behaviour. A possible solution is the following: since bootstrap nodes are linked to Validators which are block producers, they can give priority of receiving the new blocks to those peers that send valid PoMs or MPoMs.
...
One of the most resource-consuming messages is the HISTORYDIVULGE, as the partner has to provide a list of hash identifiers, one for each information he knows. This scale linearly with the number of updates. \\
The more straightforward way to decrease the size of the list is to encoded the identifiers with a smaller format and to reduce the number of identifiers. This is archivable by:
\begin{itemize}
...
The message header takes a non-trivial amount of bytes in each message. It is possible for each peer to send just the first message with the full header, while the following messages could contain just a hash of it.
...
\textbf{Rational nodes}          & 10       & Number of rational nodes                                          \\ \hline
\textbf{Byzantine nodes}         & 5       & Number of byzantine nodes                                         \\ \hline
...
Most of the metrics in the figures above, show very similar data. Our prototype has a little slower convergence rate and trades per epoch but this is probably due to small known implementation issues. \\
Still, looking at the plots gives us a high confidence that the simulation can faithfully reproduce a real environment.
...
\textbf{Rational nodes}          & Variable       & Number of rational nodes                                          \\ \hline
\textbf{Byzantine nodes}         & Variable       & Number of byzantine nodes                                         \\ \hline
...
The main goal of these experiments is to see how the protocol behaves under different assumptions such as presence of rational nodes or byzantine nodes, or both. We analyze the simulation looking at different factors such as convergence rate, number of duplicate packets, type of exchange, etc. \\
...
\hline
                                 & \textbf{Simulation One} & \textbf{Simulation Two} & \textbf{Simulation Three} \\ \hline
...
Other important factors in the simulations are the connectivity between Bootstrap nodes and the number/distribution of Full nodes by their nature.

\begin{figure}[!htb]
   \begin{minipage}{0.48\textwidth}
     \centering
     \includegraphics[width=\linewidth]{images/1/1-peer-nature-per-bn.png}
     \caption{1st simulation peer distribution and nature}
     \label{fig:sim-one-init-distribution}
   \end{minipage}\hfill
   \begin{minipage}{0.48\textwidth}
     \centering
     \includegraphics[width=\linewidth]{images/2/two-peer-nature-per-bn.png}
     \caption{2nd simulation peer distribution and nature}
...
\begin{figure}[H]
  ...
  \caption{3rd simulation peer distribution and nature}
  \label{fig:sim-three-init-distribution}
\end{figure}

As we can see from figure \ref{fig:sim-one-init-distribution}, \ref{fig:sim-two-init-distribution} and \ref{fig:sim-three-init-distribution}, each bootstrap node has more or less the same number of peer and, in the second and third simulation, also the distribution of rational and byzantine actors is even. \\
...
The first propriety we want to analyze is the convergence rate. 
...
The situation change in figure \ref{fig:sim-three-conv}, when byzantine actors are introduced. In fact, we can clearly see that more peers are having a hard time receiving updates and that 200 epochs are not enough to reach the same level of convergence of the previous situations. Still, even if more peers are having troubles, most of them have a mempool size above the mean. 
\section{Duplicates}
...
\begin{figure}[!htb]
   \begin{minipage}{0.49\textwidth}
     ...
     \includegraphics[width=\linewidth]{images/1/1-duplicates.png}
  	\caption{1st simulation duplicate updates}
  	\label{fig:sim-one-duplicates}
   \end{minipage}\hfill
   \begin{minipage}{0.49\textwidth}
     \centering
     \includegraphics[width=\linewidth]{images/2/two-duplicates.png}
       \caption{2nd simulation duplicate updates}
 		\label{fig:sim-two-duplicates}
   \end{minipage}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{3rd simulation duplicate updates}
...
Still, even if the mean in the last figure is lower than in the other two, we have to remember that the third simulation isn't able to reach full network convergence.
...
We started the first chapter by introducing what are gossip protocols, where are used and why they important, with a focus on blockchain systems. \\
In the second chapter, we started a discussion on the background needed to understand the major challenges of this work, introducing the concepts of blockchain, gossip protocols and BAR game theory model. \\
 We then moved to the third chapter, where we described in details how BBAR Gossip work, from the description of the network and environment until the message exchanges. \\
...
In conclusion, we have been able to provide a gossip protocol for blockchains considering the BAR model assumption and adopting techniques derived from BAR Gossip.
